const Cardapio = {
  ...
  apiActive: false, products: [], cart: new Map(), category: 'Todos', search: '', deliveryFee: 0, coupon: null,
  fallback: [
  ...
  change(id,delta){const qty=Math.max(0,this.qty(id)+delta);qty?this.cart.set(id,qty):this.cart.delete(id);if(this.coupon)this.clearCoupon('Carrinho alterado. Aplique o cupom novamente.');this.renderCatalog();this.renderCart()},
  ...
  payload(form){const data=new FormData(form),fulfillmentType=data.get('fulfillment');return{customer:{name:data.get('name'),phone:data.get('phone')},fulfillmentType,address:{postalCode:data.get('postalCode')||'',city:data.get('city')||'',street:data.get('street')||'',number:data.get('number')||'',district:data.get('district')||'',complement:data.get('complement')||'',reference:data.get('reference')||''},paymentMethod:data.get('payment'),couponCode:this.coupon?.code||'',notes:data.get('notes')||'',adultConfirmed:data.get('adultConfirmed')==='on',website:data.get('website')||'',items:this.cartData().map(i=>({productId:i.product.id,quantity:i.quantity}))}},
  ...
  async submit(event){event.preventDefault();const form=event.currentTarget,payload=this.payload(form),error=this.validate(payload);if(error){this.toast(error);return}const button=document.getElementById('checkout-button');button.disabled=true;
    if(this.apiActive){try{const response=await fetch(this.endpoint('/public/orders'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const body=await response.json();if(!response.ok)throw new Error(body.error);this.cart.clear();this.coupon=null;this.renderCatalog();this.renderCart();this.openCart(false);this.toast(`Pedido #${body.data.orderNumber} recebido!`);form.reset();return}catch(error){button.disabled=false;if(['coupon_already_used','first_order_only','coupon_not_found','invalid_phone'].includes(error.message)){this.coupon=null;document.getElementById('coupon-status').textContent=this.couponError(error.message);this.renderCheckout();this.toast(this.couponError(error.message));return}this.toast('O servidor não respondeu. Tente novamente.');return}}
